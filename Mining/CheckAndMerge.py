...
#For Norway, the files are called Ens_lds{nmbr}.nc, for Svalbard, the files are called Ens_lds_SV{nmbr}.nc
xr_year=xr.open_dataset(dirname+'/ensex/Extremes/Ens_lds_SV1.nc')
for year in range(2,36):
    xr_year_i=xr.open_dataset(dirname+'/ensex/Extremes/Ens_lds_SV'+str(year)+'.nc')
...
xr1=xr_1.sel(time=slice('2012-9-01','2012-12-01'),lon=slice(8,30),lat=slice(76,80)).drop(['SSTK','CI']) #drop the unnecessary variables #Norway lon=slice(4,7),lat=slice(58,63)
...
climatology_threshold=35 #Nw Climat = 90
...
xr_year.to_netcdf('//home/timok/timok/SALIENSEAS/SEAS5/ensex/Extremes/Extremes_SV.nc', mode='w')
