...
bootstrapped_array_WC_biascor= bootstrapped_series_WC * mean(obs)/mean(df_WC$V1) 
...
## Remove wrong fits
The have a very far extended x-range, because a few of the 10.000 fits result in implausible values. For example, there is one location parameter of 887176.7 (note, this might not be reproducible because the bootstrapping results are different each time). A mean precipitation value of 887.000 is of course not possible (compared to the empirical mean tests in the range 95-115). We fix a threshold above 200mm and below 0. 
Luckily, there is only one value above this threshold. There are a few (20) fits below 0. 
``` {r}
...
```
We create a new bootstrapped fit array where we apply the following criteria:
- 0 > location < 200
- log scale < 5
- X > shape < X
...
Filter the wrong fits. In addition to previous test, we set: shape > -10
``` {r}
...
trend_percent <- function(fit, vals, rp) {
  ...
  return(print(paste('Observed trend and uncertainty:', as.character(rvs_obs_trend_percent[1,2]),
