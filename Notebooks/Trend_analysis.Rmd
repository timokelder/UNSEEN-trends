...
print(paste('Highest extreme possible in comparison to observed:', as.character(max(extremes_wc)/max(obs))))
...
EVT_plot <- function(obs=obs,GEV_type) {
  ...
  fit_Seas5_biascorr <- fevd(x = extremes_wc,type=GEV_type)
  ...
  rvs_Seas5_biascorr=ci.fevd(fit_Seas5_biascorr,alpha = 0.05,type='return.level',return.period = rperiods,method ="boot")
...
# ggsave(p1,filename = "../graphs/Fidelity_EVT.png")

###Sensitivity to estimation method and extreme value distribution
#type= c("GEV", "GP", "PP", "Gumbel", "Exponential")
#method= c("MLE", "GMLE", "Bayesian", "Lmoments")
Obs_GEV_MLE <- fevd(x = obs)
# Obs_GEV_Bayes <- fevd(x = obs,method = "Bayesian")
# Obs_GEV_Bayes <- fevd(x = obs,method = "Lmoments")
Obs_Gumbel <- fevd(x = obs,type = 'Gumbel')
lr.test(Obs_Gumbel,Obs_GEV_MLE)
SEAS5_GEV <- fevd(x = extremes_wc,type='GEV') #the bias corrected version
SEAS5_Gumbel <- fevd(x = extremes_wc,type='Gumbel')
lr.test(SEAS5_Gumbel,SEAS5_GEV)
obs_test <- obs
obs_test['2005'] <- obs['2005']*1.1
# obs_test['2014'] <- obs['2014']*1.1
